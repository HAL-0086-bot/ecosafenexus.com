...
**⚠️ 切り抜き（トリミング）の鉄則 — 図を切らない**
1. UI除去の「安全範囲」は**画面ほぼ全体**にとる。除外してよいのは
   ブラウザ上部バー・下部バー/タスクバー・画面端の浮遊アイコンの帯だけ。
   **本文テキストの位置に合わせて範囲を狭めてはいけない**（図は本文列より
   大きくはみ出すことがある。実測で本文幅672pxに対し図は1487pxの例あり）。
2. 各ページは安全範囲内で内容bbox（濃い画素の外接矩形）に自動トリミングする。
3. **切れ検知を必須で行う**：bboxが安全範囲の端に接しているページが1枚でも
   あれば、そのページは切れている可能性が高い。安全範囲を広げて再実行する。
4. PDF化の前に、**図のあるページを最低1枚、目視で確認**する。
